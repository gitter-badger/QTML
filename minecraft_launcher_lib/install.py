...
def install_libraries(data: Dict[str, Any], path: str, callback: Dict[str, Callable], version_name: str = None) -> NoReturn:
    """
    ...
    for count, i in enumerate(data["libraries"]):
        ...
        if "url" in i:
            ...
            downloadUrl = "https://bmclapi2.bangbang93.com" # "https://libraries.minecraft.net" # 源
        try:
        ...
        if "downloads" not in i:
            if "extract" in i:
                extract_natives_file(os.path.join(currentPath, jarFilenameNative), os.path.join(path, "versions", (version_name if version_name else data["id"]), "natives"), i["extract"])
            continue
...
def do_version_install(versionid: str, path: str, callback: Dict[str, Callable], url: str = None, version_name: str = None) -> NoReturn:
    """
    Install the given version
    """
    # Download and read versions.json
    if url:
        download_file(url, os.path.join(path, "versions", (version_name if version_name else versionid), (version_name if version_name else versionid) + ".json"), callback)
    with open(os.path.join(path, "versions", (version_name if version_name else versionid), (version_name if version_name else versionid) + ".json")) as f:
        versiondata = json.load(f)
    # For Forge
    if "inheritsFrom" in versiondata:
        versiondata = inherit_json(versiondata, path)
    install_libraries(versiondata, path, callback, version_name=version_name)
    ...
    if "downloads" in versiondata:
        download_file(versiondata["downloads"]["client"]["url"], os.path.join(path, "versions", (version_name if version_name else versionid), (version_name if version_name else versionid) + ".jar"), callback, sha1=versiondata["downloads"]["client"]["sha1"])
    # Need to copy jar for old forge versions
    if not os.path.isfile(os.path.join(path, "versions", (version_name if version_name else versionid), (version_name if version_name else versionid) + ".jar")) and "inheritsFrom" in versiondata:
        inheritsFrom = versiondata["inheritsFrom"]
        shutil.copyfile(os.path.join(path, "versions", (version_name if version_name else versionid), (version_name if version_name else versionid) + ".jar"), os.path.join(path, "versions", inheritsFrom, inheritsFrom + ".jar"))
...
def install_minecraft_version(versionid: str, minecraft_directory: Union[str, os.PathLike], callback: Dict[str, Callable] = None, version_name: str = None) -> NoReturn:
    """
    ...
    for i in version_list["versions"]:
        if i["id"] == versionid:
            do_version_install(version_name if version_name else versionid, minecraft_directory, callback, url=i["url"], version_name=version_name)
            return
