...
def get_libraries(data: Dict[str, Any], path: str, version_name:str = None) -> str:
    """
    ...
    if "jar" in data:
        libstr = libstr + os.path.join(path, "versions", version_name if version_name else data["jar"], version_name if version_name else data["jar"] + ".jar")
    else:
        libstr = libstr + os.path.join(path, "versions", version_name if version_name else data["id"], version_name if version_name else data["id"] + ".jar")
    return libstr
...
def replace_arguments(argstr: str, versionData: Dict[str, Any], path: str, options: Dict[str, Any]) -> str:
    ...
    argstr = argstr.replace("${launcher_name}", options.get("launcherName", "qtml我的世界启动器"))
